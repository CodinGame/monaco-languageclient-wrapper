...
export class CsharpExtensionFeature extends ExtensionFeature {
  ...
  activate(context: ExtensionContext): void {
    context.subscriptions.push(
      workspace.registerTextDocumentContentProvider('omnisharp-metadata', {
        provideTextDocumentContent: async (uri: Uri, token: CancellationToken): Promise<string> => {
          const [, project, assembly, typeName] =
            /^\/Project\/(.*)\/Assembly\/(.*)\/Symbol\/(.*).cs$/.exec(uri.fsPath)!
          const { Source } = await this.languageClient.sendRequest(
            OmnisharpMetadataRequestType,
            {
              Timeout: 5000,
